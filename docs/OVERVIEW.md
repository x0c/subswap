...
| `subswap swap <id>` | 手动切换；`<id>` 全局唯一，或用 `<provider>/<id>` 消歧 |
| `subswap rm <id>` | 删除账号（registry + keyring） |
| `subswap doctor` | 环境自检 |

被砍的子命令：`add` / `list` / `quota` / `refresh` / `auto` / `daemon`（统一收进无参默认行为）。

隐藏的一次性命令:`subswap migrate-local` — 从老的 `~/.local/share/claude-swap/` 和
`~/.codex/` 把账号搬到 subswap。`--help` 里看不到,只给从老工具迁移过来的人用一次。
