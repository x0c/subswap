...
| 项 | 值 | 备注 |
|---|---|---|
| Client ID（默认） | `9d1c250a-e61b-44d9-88ed-5944d1962f5e` | 与 claude-swap 一致；公开值，非 secret |
...
- 若实时接口成功但字段不可识别，且账号来自 codex-auth 导入，subswap 可使用新鲜的
...
3. **OpenAI OAuth client_id 不是公开常量**。和 Anthropic 的 `9d1c250a-...` 不同，
   盲目硬编码风险大。
4. **codex-auth 自己也没做**。`workflows/login.zig` 只处理首次登录拿 `auth.json`，没有
   刷新流程。
...
- codex-auth 自己维护 `~/.codex/accounts/registry.json` + `accounts/<key>/auth.json`
