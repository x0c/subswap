...
因此 `add-api` 的 Kimi 预设走**两档模型选择**：强档映射 Opus/Sonnet、快档映射 Haiku/Subagent，
两档默认都用 `kimi-for-coding` 以免低档位账号选到用不了的模型而 400；非交互缺省全 `kimi-for-coding`，
`--*-model` 参数可覆盖。
...
- 实现：`crates/providers/kimi/src/kimi_usage.rs::fetch_quota`。
