...
- 触发自动切换前，如果 quota 查询失败或返回未知，不猜测、不补打请求，直接 Degraded 并提示手动 `subswap swap`。
...
| 触发条件 | 现象 | 行为 |
|---|---|---|
| 当前账号 `query_quota` 失败 | 不知道是否超额 | 不自动切换；记录 warn 日志；CLI 提示 |
| 所有候选账号 `query_quota` 失败 | 不知道选谁 | 不自动切换；提示 doctor + 手动 swap |
| 所有候选 `status == Exhausted` | 没人能用 | 不切；提示用户等重置时间或加账号 |
