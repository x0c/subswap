...
2. **可用性**：优先选择至少一个窗口明确低于自动切换阈值的账号；其他窗口耗尽不阻止切换。
...
6. **查询失败候选兜底**：当前账号已明确耗尽、且没有已知可用候选时，允许切到 `query_quota` 失败的其他账号；
   查询失败不代表账号不可用，继续停留在已耗尽账号则一定无法承接流量。active 自身仅查询失败时仍不触发切换。
...
- active 仅 quota 查询失败或返回未知时，不猜测、不补打请求，直接 Degraded 并提示手动 `subswap swap`。
