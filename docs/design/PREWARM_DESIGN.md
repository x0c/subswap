...
> 关联不变量：AGENTS.md #10、ARCHITECTURE.md §5.6（风控边界）——本功能是 #10 的**显式豁免**，实现时必须同步改这两处。

## 1. 动机

Claude 的 5h 用量窗口是「**首条消息锚定**」：你发第一条消息的那一刻窗口才起跑、5h 后重置。
多账号场景下的痛点：

- 在 A 号开工，A 的窗口 T0→T5 起跑；
- 2h 把 A 干到接近耗尽，切到从没碰过的 B —— B 的窗口**此刻才起跑**（T2→T7）；
- 于是 B 的下一次重置要等到 T7。若开工时就给 B 发过一条 hi（T0 锚定），B 会在 T5 就重置，
  **多个号的窗口错峰/对齐，连续可干活的总时长被拉长**，不易出现「所有号都还在窗口里、没一个能刷新」的空档。

预热 = 提前给闲置号发一条最小消息，把它的 5h 窗口计时器**提前拨起来**。
...
- 复用现有 `activate` 的快照/回滚（不变量 #3），任一步失败要能回到原状态。
...
**不做**：daemon 后台自动预热（churn 凭证 + 更贴 #10 精神红线，留作后续单独评估）。

## 6. 配置参数（实现时走 settings.rs，遵不变量 #9）
...
#10 禁的是「**高频**请求模拟限流 / 请求风暴」。预热是**极低频**（每号每窗口最多一次）、走官方 CLI、
仅对本人已注册账号 —— **不违反 #10 字面**，但碰了它「subswap 不主动制造与任务无关流量」的精神。
项目 owner 已决定做此功能。**实现 PR 必须同步**：

- 在 `AGENTS.md #10` 增补预热豁免边界（仅官方 CLI、仅本人号、每窗口≤1 次、失败退避、默认关、仅显式命令）；
...
单条 hi 走官方 CLI、本人付费号 → 封号风险基本为零（真正风控针对高频风暴/绕限流/共享倒卖）。
主要「代价」是它属于 subswap 主动发起的非任务流量（已作为 #10 豁免记录在案）+ 占用极少 7d 周额度。
