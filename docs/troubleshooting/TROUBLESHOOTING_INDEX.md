...
| [2026-09-05 Cursor Credits 误当成 `$20` API 池](2026-09-05-cursor-credits-zero-despite-claimed-remaining.md) | Cursor `$ [$0.00 left]` 与 Spending 页不符、把 Pro `$20`/API 当成 Credits、某号**没有显示** Credits/`$` 列（是否没余额）、或改 `get-credit-grants-balance` / 并行池自动换号前必读 |
