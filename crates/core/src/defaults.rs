...
/// 手动切换保持期（毫秒）。
///
/// 用户手动 `swap` / `login` 切换某 provider 后，该 provider 在此窗口内暂停一切自动切换
/// （连「已明确耗尽 / 达到 threshold」的确定性切换一起挡），避免把用户的显式选择掰回去。
/// 与 [`AUTO_SWAP_SETTLE_GRACE_MS`] 正交：settle 只挡 loading / 查询失败等不确定状态且不分手动自动；
/// hold 只认手动切换，但连确定性切换一起挡。`0` 或负数关闭该保护。
pub const AUTO_SWAP_MANUAL_HOLD_MS: i64 = 10 * 60 * 1000;
