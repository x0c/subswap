...
fn build_draft(options: AddApiOptions, interactive: bool) -> Result<Draft> {
    ...
    // 兼容 1.3 及更早版本的脚本：旧 `--model` 是全部角色的共同兜底，
    // 旧 `--subagent-model` 再覆盖 Haiku 轻量模型；各新角色参数始终最高优先。
    let legacy_model = options.model;
    let opus_model_option = options.opus_model.or_else(|| legacy_model.clone());
    let sonnet_model_option = options.sonnet_model.or_else(|| legacy_model.clone());
    let haiku_model_option = options
        .haiku_model
        .or(options.subagent_model)
        .or(legacy_model);
    ...
    let (opus_model, sonnet_model, haiku_model) = if preset == "kimi" && interactive {
        ...
        (
            flag_or(opus_model_option, &opus),
            flag_or(sonnet_model_option, &sonnet),
            flag_or(haiku_model_option, &haiku),
        )
    } else {
...
mod tests {
    ...
    #[test]
    fn current_model_flags_take_priority_over_legacy_aliases() {
        let draft = build_draft(
            AddApiOptions {
                preset: Some("custom".into()),
                id: Some("test".into()),
                ...
                api_key: Some("secret".into()),
                auth: Some("bearer".into()),
                opus_model: Some("current-opus".into()),
                sonnet_model: Some("current-sonnet".into()),
                haiku_model: Some("current-haiku".into()),
                model: Some("legacy-main".into()),
                subagent_model: Some("legacy-subagent".into()),
                effort: Some("max".into()),
                billing: Some("metered".into()),
                yes: true,
            },
        ...
        assert_eq!(draft.opus_model, "current-opus");
        assert_eq!(draft.sonnet_model, "current-sonnet");
        assert_eq!(draft.haiku_model, "current-haiku");
    }

    #[test]
    fn legacy_subagent_model_overrides_legacy_model_for_haiku() {
        let draft = build_draft(
            AddApiOptions {
                preset: Some("custom".into()),
                id: Some("test".into()),
        ...
        assert_eq!(draft.opus_model, "legacy-main");
        assert_eq!(draft.sonnet_model, "legacy-main");
        assert_eq!(draft.haiku_model, "legacy-subagent");
    }
}
