...
**平台支持**：已在 macOS 和 Linux 上测试并运行良好。Windows 暂未测试。
...
subswap 会把每个账号的凭证保存到应用数据目录下的 `0600` 文件（首次运行时自动迁移旧 keyring 数据），对所有读取同一套本地凭证文件的客户端原子切换当前账号，并且永远不会因为网络问题阻塞手动切换；额度查询只作为参考信息。
...
`subswap run / shell / env` 允许你在不同终端并行使用不同账号，不改全局活账号。凭证被投影到私有目录，原生 CLI 通过环境变量指向该目录（Codex 用 `CODEX_HOME`；Claude 在 macOS 上用 `CLAUDE_CONFIG_DIR` + `CLAUDE_SECURESTORAGE_CONFIG_DIR`）。
...
- **独占锁**：同一账号同时只能被一个隔离会话借走，防止 refresh token 轮换冲突。
...
暂未在 Windows 上测试。macOS 和 Linux 是主要支持的平台。
